        ...
        <div class="w-100 h-100">
            <table class="table table-bordered text-center">
                ...
                <tbody class="text-white">
                    @forelse ($setor1 as $vaga)
                        <tr class={{ $vaga['_value'] ? 'bg-success' : 'bg-danger' }}>
                            <td><span class="text-stroke"><b>{{ $vaga['sensor'] }}</b></span></td>
                            <td><span class="text-stroke"><b>{{ $vaga['_value'] ? 'Disponível' : 'Indisponível' }}</b></span></td>
                        </tr>
                    @empty
                        <tr class="text-dark">
                            <td colspan="2" class="text-stroke"><b>Nenhuma vaga disponível no momento</b></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
